document.addEventListener("DOMContentLoaded", function () {
  ...
  for (let i = 0; i < singleAttribute.length; i++) {
    ...
    if (singleAttribute[i][0].includes('Рекомендованная площадь помещения')) {
      square = singleAttribute[i][0][2];
    }
  }

  // Показать площадь помещения
  const areaItem = document.getElementsByClassName('area-item')[0]
  const area = document.getElementsByClassName('area')[0]
  if (square) {
    area.classList.remove('d-none');
    areaItem.innerHTML = square;
  }
