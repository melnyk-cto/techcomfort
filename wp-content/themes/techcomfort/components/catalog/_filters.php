<div class='filter'>
    ...
    <?php foreach ($attributeArray as $key => $values) {
        ...
        if (
            $key === 'Макс. к-во подключаемых внутренних блоков' ||
            $key === 'Расход воздуха, м3/ч' ||
            $key === 'Холодопроизводительность (кВт)' ||
            ...
            $key === 'Размеры наружного блока, Ш (мм)' ||
            $key === 'Размеры наружного блока, В (мм)' ||
            $key === 'Размеры наружного блока, Г (мм)' ||
